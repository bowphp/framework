...
class Loader
{
     ...
     * @return mixed
     */
    public static function launch($cb, $param = null, array $names = [])
    {
        ...
        if (is_string($middleware)) {
            if (!in_array(ucfirst($middleware), $names['middlewares'], true)) {
                throw new ClassLoaderException($middleware . ' n\'est pas un middleware définir.', E_ERROR);
            }
            ...
            if (!class_exists($classMiddleware)) {
                throw new ClassLoaderException($middleware . ' n\'est pas un class Middleware.');
            }
     ...
     * @return mixed|void
     */
    private static function execute($arr, $arg)
    {
        if (is_callable($arr)) {
